...
public class HW2Operations
{
	 ...
	 *  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	 * 1010101010100101 43685 0xAAA5
	 * 1010101010100001 43681 0xAAA1
	 * 0101011010110010 0x56B2
	 * 1111011010110010 0xF6B2
	 * Examples:
	...
	{
		// by logic, True AND q = q, False or q = q.
		// if we were to change abcd to aecd. which = 2, nibble = e.
		// let abcd OR FTFF "false,true,false,false", this becomes aTcd.
		// let aTcd AND TeTT "true,e,true,true", this becomes aecd.
		// here ~0x000F << (4*which) would give us FTFF.
		// nibble << (4*which) would give us FeFF.
		// FeFF | (~FTFF) --> TeTT 
		int sMask = (0x000F << (which << 0x02));
		int sNibble = nibble << (which << 0x02);
        return ( num | sMask ) & (sNibble | ~sMask);
	}
	...
	public static int getShort(int num, int which)
	{
		int output;
		if (which == 0) {
			output = ~(0x0000FFFF << 0x10) & num;
		}  else {
			output = (num  >> 0x10 & 0xFFFF);
		}
		return output;
	}
	...
	{
		return (n1 << 0x0C)|(n2 << 0x08)|(n3 << 0x04)|n4;
	}
	...
	{
		int sMask = ~(0xFFFFFFFF << n);
		int twoCom = num & sMask;
		int sign = (twoCom >> (n-0x00000001)) & 0x00000001;
		if (sign == 0x00000001) {
			return (twoCom ^ sMask) + 0x00000001;		
		}
		return twoCom;
	}
